...
def collection():
    ...
    if user == "" and passw == "":
        ...
        if os.path.exists("iteration1/dates.txt"):
            with open("iteration1/dates.txt", "r") as file:
                for line in file:
                    dates_data, username = line.split(" : ")
                    year, month, day, is_recycling = dates_data.split("-")
                    print(username, dates_data)
                    if username == user:
...
def calculate_day(day, month, is_recycling, c_frame):
    ...
    if user != "" and passw != "":
        ...
        with open("iteration1/dates.txt", "w") as file:
...
def delete_account():
    ...
    confirm = Button(d_frame, text="Yes", command= lambda:[d_frame.destroy(), main(), os.remove("iteration1/info.txt")])
    confirm.pack()
...
def create_account(username, password, error_message, r_frame):
    if username == "" or password == "":
        ...
        return
    r_frame.destroy()
    with open("iteration1/info.txt", "a") as file:
